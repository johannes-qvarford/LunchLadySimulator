	...
	IEnumerator SpawnFood()
	{
		while(true)
		{
			if(quantityOfFood < items)
			{
				...
				{
					tries ++;
					spawnPosTest = new Vector3(Random.Range(spawnZone.transform.position.x-(spawnZone.collider.bounds.size.x/2),spawnZone.transform.position.x+(spawnZone.collider.bounds.size.x/2)),
					...
					Collider[] temp = Physics.OverlapSphere(spawnPosTest,spawnObject.GetComponent<BoxCollider>().size.magnitude,mask.value);
					Debug.Log(spawnObject.GetComponent<BoxCollider>().size.magnitude);
					if(temp.Length < 1)
					{
						foundPos = true;
